...
def main():
    ...
    if not args.use_mamba2:
        model = LLM(
            ...
            disable_custom_all_reduce=True,  # todo: need check
        )
    else:
        model = LLM(
            model=args.model_path,
    ...
    for _ in range(args.n):
        ...
        with torch.no_grad():
            ...
            while pending_prompts and retry_count < args.max_retries:
                ...
                batch_texts = [completion.outputs[0].text.strip() for completion in batch_outputs]
                batch_texts = [f"<think>\n{text}" if not text.startswith("<think>") else text for text in batch_texts]
